...
async def multiplier_2times2_test(dut):
    ...
    assert dut.q.value == 1, (
        f"Div result is incorrect: A={dut.a.value}, B={dut.b.value} {dut.y.value} != 4"
    )


@cocotb.test()
async def multiplier_5dividedby10_test(dut):
    """Test for 5 + 10"""
    ...
    assert dut.q.value == 0, (
        f"Div result is incorrect: A={dut.a.value}, B={dut.b.value} {dut.y.value} != {0}"
    )
...
async def anything_dividedby1_test(dut):
    ...
    assert dut.q.value == A, (
        f"Critical Error: ANYTHING DIVIDED BY ONE IS NOT ITSELF! A={dut.a.value}, B={dut.b.value} {dut.y.value} != A"
    )
